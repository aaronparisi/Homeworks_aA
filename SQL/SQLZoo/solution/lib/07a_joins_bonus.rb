...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT styles.style, (SUM(album_track_counts.price) / SUM(album_track_counts.count))
    FROM styles
    JOIN (
    ...
    ORDER BY (SUM(album_track_counts.price) / SUM(album_track_counts.count)) DESC, styles.style ASC
    LIMIT 5
  SQL
end
