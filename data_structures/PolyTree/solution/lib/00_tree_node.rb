...
class PolyTreeNode
  ...
  def children
    ...
    # so they are NOT making attr_reader :children
    # this method ensures that every time someone asks for children,
    # they are getting a DUPLICATE of the children
    @children.dup
  end

  def parent=(parent)
    return if self.parent == parent
    # "if the given parent matches the current parent, do nothing at all"
    # my code will go about removing the current node from the parent's children,
    # only to add it back in after reassigning

    # First, detach from current parent.
    if self.parent
      # if the current node does not have a nil parent,
      self.parent._children.delete(self)
      # I don't think you need to call remove_child??
    end

    @parent = parent
    self.parent._children << self unless self.parent.nil?
    # only add to the parent's children array if there actually is a parent
